...
pub enum DnsError {
    ...
    #[error("invalid DNS response: not a query for _origin.z32encodedpubkey")]
    InvalidResponse {},
}
...
impl DnsResolver {
    ...
    pub async fn lookup_endpoint_by_id(
        ...
        let name = endpoint_info::ensure_origin_txt_label(name);
    ...
    pub async fn lookup_endpoint_by_domain_name(
        ...
        let name = endpoint_info::ensure_origin_txt_label(name.to_string());
        let lookup = self.lookup_txt(name.clone(), DNS_TIMEOUT).await?;
        let info = EndpointInfo::from_txt_lookup(name, lookup)?;
        Ok(info)
    }
